...
var top_speed = 0.4;
var score = 0
var ncoins = 50;   // there are 30 coins
var lives = 2     // there are 5 lives
...
var mc = new Hammer(myElement);    // hammer ??

//enable all directions
mc.get('swipe').set({
  direction: Hammer.DIRECTION_ALL,     // This allows swipes to be detected in any direction (up, down, left, right).
  threshold: 1,
  velocity:0.1     // swipe speed (in pixels per millisecond)
});
...
function main() {
  ...
  walls1.init(gl,[0,0,20]);
  walls2.init(gl,[500,200,-120]);
  back.init(gl,[0,0,0]);
  surfer.init(gl,[0,-1.2,-3]);
  police.init(gl,[0,-1.5,-1]);
  ...
  for(i=0;i<ncoins;++i){
    ...
    x = track();
    c = new Coin();
    c.init(gl, [x[0], -1+2*x[1], -5*i-4*x[1]]);
    coins.push(c);
  }
  for(i=0;i<ncoins;++i){
  ...
  /////////////////////////jetpacks
      for(i=0;i<jetpacks.length;i++){
        ...
        if(jetpacks[i].pos[2]>=5){
          ...
          gl = jetpacks[i].draw(gl, deltaTime, 
            projectionMatrix, modelViewMatrix, programInfo);
        }
      }
